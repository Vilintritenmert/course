...
ConfigLoaderOptions::ConfigLoaderOptions(std::string mainConfig,
    ...
    : _config(mainConfig), _ammoParams(ammoConfig), _targetConfig(targetConfig),
      _ballistikTable(ballistikTable), _resultPath(resultPath) {}
...
void JsonConfigLoader::loadAmmoConfig(const std::string &path) {
  try {
    ...
    _ammoConfig = std::make_shared<AmmoConfig>(ammoItems);
  } catch (std::exception error) {
    throw ConfigLoaderException("Failed load config: `" + path + "`");
  }
}
...
void JsonConfigLoader::loadDronConfig(const std::string &path) {
  try {
    json jsonData = this->loadFile(path);
    _mainConfig = std::make_shared<Config>(
        Position(jsonData["drone"]["position"]["x"].get<float>(),
...
void JsonConfigLoader::loadConfig(
    const std::shared_ptr<ConfigLoaderOptions> configLoaderOptions) {
...
std::shared_ptr<AmmoConfig> JsonConfigLoader::getAmmoConfig() const { return _ammoConfig; }

std::shared_ptr<Config> JsonConfigLoader::getConfig() const { return _mainConfig; }
